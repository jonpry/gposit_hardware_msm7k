...
static int createGLTexture(int format)
{
    GLuint   text;
    int i, j, base, potSize;
 ...
 /*   if(!gpu1_base)
    ...
    unsigned char* data = (unsigned char*)malloc(128*128*bpp);
    for(i=0; i < potSize*potSize*bpp; i++)
    {
       data[i] = rand();
    }
...
init_texture_access();
    ...
    //Lets find it
    base=0;
    for(i=0x2F0000; i < 8*1024*1024-potSize*potSize*bpp; i++)
    {
	base=i;
 	for(j=0; j < potSize*potSize*bpp; j++)
	{
...
try_ashmem:
        ...
        if (err == 0) {
	    LOGE("About to call ogl_alloc at 0x%p", ogl_alloc);
	    if(ogl_alloc)
		    ogl_alloc(w,h,format);
